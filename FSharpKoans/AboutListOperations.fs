    ...
    *)
    ...
    let ``01 Finding the length of a list, the hard way`` () =
        let length (xs : 'a list) : int =
            let rec counter xs count =
                match xs with
                | [] -> count
                | _::tail -> counter tail count + 1
            counter xs 0
    ...
    let ``03 Reversing a list, the hard way`` () =
        let rev (xs : 'a list) : 'a list =
            let rec method xs (newlist : 'a list) =
                match xs with
                | [] -> newlist
                | a::b -> method b (a::newlist)
            method xs []
    ...
    let ``05 Fixed-function mapping, the hard way (part 1).`` () =
        let map (xs : int list) : int list =
            let rec mapper xs newlist =
                match xs with 
                | [] -> newlist
                | a::b -> mapper b (a+1::newlist)
            List.rev (mapper xs [])
    ...
    let ``06 Fixed-function mapping, the hard way (part 2).`` () =
        let map (xs : int list) : int list =
            let rec double xs newlist =
                match xs with 
                | [] -> newlist
                | a::b -> double b (a*2::newlist)
            List.rev (double xs [])
   ...
   *)
    ...
    let ``07 Specified-function mapping, the hard way`` () =
        let map (f : 'a -> 'b) (xs : 'a list) : 'b list =
            let rec apply (xs : 'a list) (newlist) =
                match xs with
                | [] -> newlist
                | a::b -> (apply b) ((f a)::newlist)
            List.rev (apply xs [])
...
*)
    ...
    let ``14 A fold which sums a list`` () =
        let fold initialState xs =
            let rec apply xs state =
                match xs with
                | [] -> state
                | a::b -> apply b (state + a)
            apply xs initialState
    ...
    let ``20 init: creating a list based on a size and a function`` () =
        let init (n : int) (f : int -> 'a) : 'a list =
            let rec apply x newlist =
                match x < n with
                | false -> newlist
                | true -> apply (x + 1) ((f x)::newlist)
            List.rev (apply 0 [])
    ...
    let ``23 choose: find all matching elements, and transform them`` () =
        ...
        let choose (p : 'a -> 'b option) (xs : 'a list) : 'b list =
            let rec apply xs newlist =
                ...
                | a::b ->
                    match p a with
                    | None -> apply b newlist
                    | Some c -> apply b (c::newlist)
            List.rev (apply xs [])
